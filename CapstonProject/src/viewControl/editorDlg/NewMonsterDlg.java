...
public class NewMonsterDlg extends EditorDlg implements ActionListener {

	private static final long serialVersionUID = 1L;
	
	private String loadFileName;
	private MonsterEditorSystem monsterEditSystem;
	...
	public NewMonsterDlg(MainFrame parent, boolean isNew, String fileName) {
		super(parent, "Edit Character");

		loadFileName = fileName;
		this.isNew = isNew;
		monsterEditSystem = new MonsterEditorSystem(projectPath);
		
		// 새롭게 생성하는 것이면 charEditSystem을 초기화하여 생성하고
		// 그렇지 않다면 경로를 통해 Object를 읽어들여 내용을 복사한다.
		if (!isNew) {
			try {
				monsterEditSystem.load(loadFileName);
	...
	private void initComponents() {
		...
		// JTextField 변수
		tf_charName = new JTextField(monsterEditSystem.getName(), 15);
		tf_initLevel = new JTextField((new Integer(monsterEditSystem.getInitLevel())).toString(), 3);
		tf_maxLevel = new JTextField((new Integer(monsterEditSystem.getMaxLevel())).toString(), 3);
		
		tf_initHP = new JTextField((new Integer(monsterEditSystem.getInitAbility().getHP())).toString(), 3);
		tf_initMP = new JTextField((new Integer(monsterEditSystem.getInitAbility().getMP())).toString(), 3);
		tf_initEXP = new JTextField((new Integer(monsterEditSystem.getInitAbility().getEXP())).toString(), 3);
		tf_initStrength = new JTextField((new Integer(monsterEditSystem.getInitAbility().getStrength())).toString(), 3);
		tf_initVitality = new JTextField((new Integer(monsterEditSystem.getInitAbility().getVitality())).toString(), 3);
		tf_initIntelligence = new JTextField((new Integer(monsterEditSystem.getInitAbility().getIntelligence())).toString(), 3);
		tf_initKnowledge = new JTextField((new Integer(monsterEditSystem.getInitAbility().getKnowledge())).toString(), 3);
		tf_initAgility = new JTextField((new Integer(monsterEditSystem.getInitAbility().getAgility())).toString(), 3);
		
		tf_growthCurveHP = new JTextField((new Integer(monsterEditSystem.getGrowthCurve().getHP())).toString(), 3);
		tf_growthCurveMP = new JTextField((new Integer(monsterEditSystem.getGrowthCurve().getMP())).toString(), 3);
		tf_growthCurveEXP = new JTextField((new Integer(monsterEditSystem.getGrowthCurve().getEXP())).toString(), 3);
		tf_growthCurveStrength = new JTextField((new Integer(monsterEditSystem.getGrowthCurve().getStrength())).toString(), 3);
		tf_growthCurveVitality = new JTextField((new Integer(monsterEditSystem.getGrowthCurve().getVitality())).toString(), 3);
		tf_growthCurveIntelligence = new JTextField((new Integer(monsterEditSystem.getGrowthCurve().getIntelligence())).toString(), 3);
		tf_growthCurveKnowledge = new JTextField((new Integer(monsterEditSystem.getGrowthCurve().getKnowledge())).toString(), 3);
		tf_growthCurveAgility = new JTextField((new Integer(monsterEditSystem.getGrowthCurve().getAgility())).toString(), 3);
		...
		if (!this.isNew) {
			cb_indexChar.setSelectedIndex(monsterEditSystem.getIndex());
			cb_weapon.setSelectedIndex(getSelectedEquipment(0, 0, monsterEditSystem.getIndexInitWeapon()));
			cb_topArmor.setSelectedIndex(getSelectedEquipment(1, 0, monsterEditSystem.getIndexInitTopArmor()));
			cb_bottomsArmor.setSelectedIndex(getSelectedEquipment(1, 1, monsterEditSystem.getIndexInitBottomsArmor()));
			cb_helmetArmor.setSelectedIndex(getSelectedEquipment(1, 2, monsterEditSystem.getIndexInitHelmetArmor()));
			cb_shieldArmor.setSelectedIndex(getSelectedEquipment(1, 3, monsterEditSystem.getIndexInitShieldArmor()));
			cb_accessoryArmor.setSelectedIndex(getSelectedEquipment(1, 4, monsterEditSystem.getIndexInitAccessoryArmor()));
		}
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btn_OK) {
			if(tf_charName.getText().compareTo("") != 0) {
				...
				monsterEditSystem.setIndex(cb_indexChar.getSelectedIndex());
				monsterEditSystem.setName(tf_charName.getText());
				try {
					monsterEditSystem.setInitLevel(initLevel);
					monsterEditSystem.setMaxLevel(maxLevel);
				} catch (IllegalLevelNumber e1) {
					e1.printStackTrace();
				}
				monsterEditSystem.setInitAbility(initHP, initMP, initEXP, initStrength, initVitality, initIntelligence, initKnowledge, initAgility);
				monsterEditSystem.setGrowthCurve(increaseHP, increaseMP, increaseEXP, increaseStrength, increaseVitality, increaseIntelligence, increaseKnowledge, increaseAgility);
				
				monsterEditSystem.setIndexJob(cb_indexJob.getSelectedIndex());
				monsterEditSystem.setSpeed(new Integer(tf_speed.getText()));
				
				// 장비는 선택된 인덱스가 저장되는 것이 아니라 선택된 장비의 인덱스가 저장된다.
				// 다른 콤보박스와 달리 선택된 인덱스와 장비의 인덱스가 다를 수 있기 때문이다.
				if(cb_weapon.getSelectedItem() != null)
					monsterEditSystem.setIndexInitWeapon(new Integer(((String)(cb_weapon.getSelectedItem())).substring(0, 3)));
				if(cb_topArmor.getSelectedItem() != null)
					monsterEditSystem.setIndexInitTopArmor(new Integer(((String)(cb_topArmor.getSelectedItem())).substring(0, 3)));
				if(cb_bottomsArmor.getSelectedItem() != null)
					monsterEditSystem.setIndexInitBottomsArmor(new Integer(((String)(cb_bottomsArmor.getSelectedItem())).substring(0, 3)));
				if(cb_helmetArmor.getSelectedItem() != null)
					monsterEditSystem.setIndexInitHelmetArmor(new Integer(((String)(cb_helmetArmor.getSelectedItem())).substring(0, 3)));
				if(cb_shieldArmor.getSelectedItem() != null)
					monsterEditSystem.setIndexInitShieldArmor(new Integer(((String)(cb_shieldArmor.getSelectedItem())).substring(0, 3)));
				if(cb_accessoryArmor.getSelectedItem() != null)
					monsterEditSystem.setIndexInitAccessoryArmor(new Integer(((String)(cb_accessoryArmor.getSelectedItem())).substring(0, 3)));
	
				// 객체를 파일로 저장한다. 내부적으로 같은 인덱스의 파일을 전부 삭제해준다.
				result = monsterEditSystem.save();
			...
			new SetAnimationDlg(owner, monsterEditSystem.getMoveUpAnimation());
			
		} else if (e.getSource() == btn_moveDownAnimation) {
			new SetAnimationDlg(owner, monsterEditSystem.getMoveDownAnimation());
			
		} else if (e.getSource() == btn_moveRightAnimation) {
			new SetAnimationDlg(owner, monsterEditSystem.getMoveRightAnimation());
			
		} else if (e.getSource() == btn_moveLeftAnimation) {
			new SetAnimationDlg(owner, monsterEditSystem.getMoveLeftAnimation());
			
		} else if (e.getSource() == btn_battleMoveUpAni) {
			new SetAnimationDlg(owner, monsterEditSystem.getBattleMoveUpAni());
			
		} else if (e.getSource() == btn_battleMoveDownAni) {
			new SetAnimationDlg(owner, monsterEditSystem.getBattleMoveDownAni());
			
		} else if (e.getSource() == btn_battleMoveRightAni) {
			new SetAnimationDlg(owner, monsterEditSystem.getBattleMoveRightAni());
			
		} else if (e.getSource() == btn_battleMoveLeftAni) {
			new SetAnimationDlg(owner, monsterEditSystem.getBattleMoveLeftAni());
			
		} else if (e.getSource() == btn_attackUpAnimation) {
			new SetAnimationDlg(owner, monsterEditSystem.getAttackUpAnimation());
			
		} else if (e.getSource() == btn_attackDownAnimation) {
			new SetAnimationDlg(owner, monsterEditSystem.getAttackDownAnimation());
			
		} else if (e.getSource() == btn_attackRightAnimation) {
			new SetAnimationDlg(owner, monsterEditSystem.getAttackRightAnimation());
			
		} else if (e.getSource() == btn_attackLeftAnimation) {
			new SetAnimationDlg(owner, monsterEditSystem.getAttackLeftAnimation());
			
		} else if (e.getSource() == btn_damageUpAnimation) {
			new SetAnimationDlg(owner, monsterEditSystem.getDamageUpAnimation());
			
		} else if (e.getSource() == btn_damageDownAnimation) {
			new SetAnimationDlg(owner, monsterEditSystem.getDamageDownAnimation());
			
		} else if (e.getSource() == btn_damageRightAnimation) {
			new SetAnimationDlg(owner, monsterEditSystem.getDamageRightAnimation());
			
		} else if (e.getSource() == btn_damageLeftAnimation) {
			new SetAnimationDlg(owner, monsterEditSystem.getDamageLeftAnimation());
			
		} else if (e.getSource() == btn_skillAnimation) {
			new SetAnimationDlg(owner, monsterEditSystem.getSkillAnimation());
			
		} else if (e.getSource() == btn_dieAnimation) {
			new SetAnimationDlg(owner, monsterEditSystem.getDieAnimation());
			
		}  else if (e.getSource() == btn_eventAnimationList) {
			EventAniDlg dlg = new EventAniDlg(owner, "Edit Event Animation List", monsterEditSystem.getEventAnimationList());
			if (monsterEditSystem.getEventAnimationList().size() == 0)
				monsterEditSystem.setEventAnimationList(dlg.getEventAnimationList());
		}
	}

	public MonsterEditorSystem getMonsterEditSystem() {
		return monsterEditSystem;
	}
	...
	private int getSelectedEquipment(int equipType, int armorType, int equipmentIndex) {
		...
		for (int i=0; i < fileList.length; i++) {
			...
			if (equipType == 0) {
				...
				if(monsterEditSystem.getIndexJob() == ((Weapons)equip).getIndexJob())
					indexComboBox++;
				...
				if (   armorType == ((Armors)equip).getEquipmentType()
					&& monsterEditSystem.getIndexJob() == ((Armors)equip).getIndexJob())
						indexComboBox++;
